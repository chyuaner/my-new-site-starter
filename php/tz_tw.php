...
<table width="100%" cellpadding="3" cellspacing="0" align="center">
    <tr><th colspan="3">網絡使用狀況</th></tr>
<?php for ($i = 2; $i < count($strs); $i++ ) : ?>
<?php preg_match_all( "/([^\s]+):[\s]{0,}(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/", $strs[$i], $info );?>
     <tr>
        <td width="13%"><?php echo $info[1][0]?> : </td>
        <td width="43%">已接收 : <font color='#CC0000'><span id="NetInput<?php echo $i?>">0</span></font></td>
        <td width="43%">已發送 : <font color='#CC0000'><span id="NetOut<?php echo $i?>">0</span></font></td>
    </tr>
<?php endfor; ?>
</table>
<?php endif; ?>
